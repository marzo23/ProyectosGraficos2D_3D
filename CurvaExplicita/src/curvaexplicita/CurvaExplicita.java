 ...
 * @author L440
 */
public class CurvaExplicita extends JFrame implements KeyListener, Runnable{
    int width = 1000;
    int height = 1000;
    static boolean automatic = false;
    public static void main(String[] args) {
        CurvaExplicita r = new CurvaExplicita();
        if(automatic)
            r.run();
    }
    ...
    public void paint(Graphics g){
        ...
        UnaryOperator<GraphLib3D.Point3D> fn = (p)->{
            return new GraphLib3D.Point3D((int)(p.x-3*Math.sin(p.x))*3,(int)(4-3*Math.cos(p.x))*3,0);
        };
        g3D.drawRotatedPoints(pInit, pDisplay, 30, degrees, g3D.getPointsExplicitCurve(fn, 100, 1));
